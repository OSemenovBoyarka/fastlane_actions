    ...
    class PrepareSnapfilesAction < Action
      def self.run(params)
        ...
        Dir.chdir(FastlaneFolder.path) do
          template = File.read((params[:snapfile_template_path]).to_s)
        ...
        [
          FastlaneCore::ConfigItem.new(key: :devices,
                                       ...
                                       verify_block: proc do |value|
                                         raise "No Devices for PrepareSnapfilesAction given, pass using `devices: ['iPhone 4s', 'iPhone 6',...]`".red unless value && !value.empty?
                                       end),
          FastlaneCore::ConfigItem.new(key: :languages,
                                       ...
                                       is_string: false,
                                       verify_block: proc do |value|
                                                       raise "No languages for PrepareSnapfilesAction given, pass using `languages: ['en-US', 'ru-RU']`".red unless value && !value.empty?
                                                     end),
